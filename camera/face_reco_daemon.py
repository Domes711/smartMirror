...
def recognize_stream(picam, max_duration, known_encodings, known_names, tolerance):
    ...
    while True:
        ...
        if not locations:
            log.info("frame %d: no face (detect %.0f ms)", frames_checked, detect_ms)
            continue

        detected_unknown = True

        # Encode faces - model="small" uses 5 landmarks (~3x faster than default 68)
        t_encode_start = time.time()
        encodings = face_recognition.face_encodings(frame, locations, model="small")
        encode_ms = (time.time() - t_encode_start) * 1000.0
        log.info("frame %d: %d face(s), detect %.0f ms, encode %.0f ms",
                 frames_checked, len(locations), detect_ms, encode_ms)
        if not encodings:
            continue
...
class FaceRecoDaemon:
    ...
    def _init_camera(self):
        ...
        try:
            ...
            # 320x240 balances detection range vs speed on Pi
            # (160x120 was too small: HOG min face ~80px needs face filling 2/3 frame)
            ...
            log.info("Camera initialized and ready (320x240, optimized)")
        except Exception as exc:
